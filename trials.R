...
ui <- dashboardPage(
  dashboardHeader(title = "DadApp: WebApp for Assigning Paternity"),
  ...
  dashboardBody(
    ...
    fluidRow(
      box(plotOutput("lodScoreplot")),
      
      box(
        title = "Parameters",
        sliderInput("fathers", "Number of Potential Fathers:", 1, 200, 100),
        sliderInput("loci", "Number of loci", 1, 30, 10),
        sliderInput("allele", "Allele Frequency", 0.1, 1, 0.5)
      )
    )
  )
)
